...
def add_prod(sellID,name,qty,category,price,desp):
    conn = sqlite3.connect("OnlineShop/onlineshop.db")
    cur = conn.cursor()
    prodID = gen_prodID()
    tup = (prodID,
...
def place_order(prodID, custID, qty, method,pstatus,cardname,cardnumber,expmonth,expyear,cvv):
    conn = sqlite3.connect('OnlineShop/onlineshop.db')
    cur = conn.cursor()
    orderID = gen_orderID()
    cur.execute("""INSERT INTO orders
                    SELECT ?,?,?,?,datetime('now'), cost_price*?, sell_price*?,?,?,'PLACED',?,?,?,?,?
                    FROM product WHERE prodID=? """, (orderID, custID, prodID, qty, qty, qty, method,pstatus,cardname,cardnumber,expmonth,expyear,cvv,prodID))
    conn.commit()
    conn.close()
...
def cust_orders(custID):
    conn = sqlite3.connect('OnlineShop/onlineshop.db')
    cur = conn.cursor()
    a = cur.execute("""SELECT o.orderID, o.prodID, p.name, o.quantity, o.sell_price, o.date, o.status, o.method, o.pstatus
...
def sell_orders(sellID):
    conn = sqlite3.connect('OnlineShop/onlineshop.db')
    cur = conn.cursor()
    a = cur.execute(""" SELECT o.orderID, o.prodID, p.name, o.quantity, p.quantity, o.cost_price, o.date, o.status, o.method, o.pstatus, o.cardname,o.cardnumber,o.expmonth,o.expyear,o.cvv
                        FROM orders o JOIN product p
...
def cust_purchases(custID):
    conn = sqlite3.connect('OnlineShop/onlineshop.db')
    cur = conn.cursor()
    a = cur.execute("""SELECT o.prodID, p.name, o.quantity, o.sell_price, o.date, o.method
                       FROM orders o JOIN product p
                       WHERE o.prodID=p.prodID AND o.custID=? AND o.status='RECIEVED'
                       ORDER BY o.date DESC """, (custID,))
    res = [i for i in a]
...
def sell_sales(sellID):
    conn = sqlite3.connect('OnlineShop/onlineshop.db')
    cur = conn.cursor()
    a = cur.execute("""SELECT o.prodID, p.name, o.quantity, o.sell_price, o.date, o.custID, c.name,o.method
