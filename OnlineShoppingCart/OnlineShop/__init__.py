...
def view_profile(id):
    ...
    det, categories = fetch_details(id, profile_type) 
    if len(det)==0:
        abort(404)
    det = det[0]
...
def seller_products(id):
    ...
    if session["type"]=="Seller":
        abort(403)
    det, categories = fetch_details(id, "Seller")   
    if len(det)==0:
        abort(404)
    det = det[0]
    name=det[1]
...
def add_products():
    ...
    if request.method=="POST":
        ...
        add_prod(session['userid'],name,qty,category,price,desp)
    ...
    if request.method=="POST":
        ...
        if choice=="PLACE ORDER":
            if payment=="NB":
                ...
                place_order(id, session['userid'], qty, method, pstatus, cardname, cardnumber, expmonth, expyear, cvv)
